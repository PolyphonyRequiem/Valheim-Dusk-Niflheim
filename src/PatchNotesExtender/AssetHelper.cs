    ...
    public static class AssetHelper
    {
        ...
        public static Recipe BuildRecipe(RecipeStub rs, ObjectDB odb)
        {
            ...
            if (recipe.m_craftingStation == null)
            {
                ...
                if (recipe.m_craftingStation == null)
                {
                    PatchNotesExtender.Log.LogInfo("BuildRecipe couldn't find crafting station " + rs.CraftingStation + " for " + rs.Name);
                    return null;
                }
            }
            if (!string.IsNullOrWhiteSpace(rs.RepairStation))
            {
                ...
                if (recipe.m_repairStation == null)
                {
                    ...
                    if (recipe.m_repairStation == null)
                    {
                        PatchNotesExtender.Log.LogInfo("BuildRecipe couldn't find declared repair station " + rs.RepairStation + " for " + rs.Name);
                        return null;
                    }
                }
            ...
            foreach (RecipeStub.RequirementStub r in rs.Requirements)
            {
                ...
                if (!id)
                {
                    PatchNotesExtender.Log.LogInfo("BuildRecipe couldn't get requirement component " + r.Component + " for " + rs.Name);
                    return null;
                }
        ...
        static void PopulateObjectDB(ObjectDB odb)
        {
            ...
            if (RecipeStubs.Count > 0 && !odb.GetRecipe(RecipeStubs[0].Item.m_itemData))
            {
                foreach (var rs in RecipeStubs)
                {
                    ...
                    {
                        odb.m_recipes.Add(r);
                        PatchNotesExtender.Log.LogInfo("Added recipe " + r.name);
                    }
                }
            }
        ...
        public static void UpdateRecipes()
        {
            foreach (var rs in RecipeStubs)
            {
                ...
                if (r)
                {
                    ...
                    PatchNotesExtender.Log.LogInfo("Updated recipe " + r.name);
                }
            }
        }
